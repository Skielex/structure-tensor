...
def structure_tensor_3d(volume, sigma, rho, out=None):
    ...
    Vx = ndimage.gaussian_filter(volume, sigma, order=[0, 0, 1], mode='nearest')
    Vy = ndimage.gaussian_filter(volume, sigma, order=[0, 1, 0], mode='nearest')
    Vz = ndimage.gaussian_filter(volume, sigma, order=[1, 0, 0], mode='nearest')
    ...
    ndimage.gaussian_filter(tmp, rho, mode='nearest', output=S[0])
    np.multiply(Vy, Vy, out=tmp)
    ndimage.gaussian_filter(tmp, rho, mode='nearest', output=S[1])
    np.multiply(Vz, Vz, out=tmp)
    ndimage.gaussian_filter(tmp, rho, mode='nearest', output=S[2])
    np.multiply(Vx, Vy, out=tmp)
    ndimage.gaussian_filter(tmp, rho, mode='nearest', output=S[3])
    np.multiply(Vx, Vz, out=tmp)
    ndimage.gaussian_filter(tmp, rho, mode='nearest', output=S[4])
    np.multiply(Vy, Vz, out=tmp)
    ndimage.gaussian_filter(tmp, rho, mode='nearest', output=S[5])
...
def eig_special_3d(S, full=False):
    ...
    d = np.product(B03, axis=0, out=tmp[3])
    ...
    d_tmp *= B1
    d -= d_tmp
    np.product(B36, axis=0, out=d_tmp)
    d_tmp *= 2
    d += d_tmp
    ...
    np.add.outer([2 * np.pi / 3, 4 * np.pi / 3], phi, out=val[:2])
    val[2] = phi
    np.cos(val, out=val)
    p *= 2
    val *= p
